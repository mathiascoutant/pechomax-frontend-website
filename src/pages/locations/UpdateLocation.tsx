...
function UpdateMessage() {
  ...
  useEffect(() => {
    const fetchid = async () => {
      try {
        const response = await axios.get<LocationData>(`http://localhost:3000/locations/${id}`,{ withCredentials: true });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3000/locations/update/${locations?.id}`, locations,{ withCredentials: true });
                ...
                <div className='grid grid-cols-2 bg-white rounded-md  p-2 gap-4'>
                  ...
                  {locations && (
                    <div>
                      ...
                      <input className='mb-4' type="text" value={locations.name ?? ''} placeholder="name" onChange={e => setLocation({ ...locations, name: e.target.value })} /> <br />
                      <input className='mb-4' type="text" value={locations.description ?? ''} placeholder="description" onChange={e => setLocation({ ...locations, description: e.target.value })} />
