            ...
            <div className='bg-slate-100 p-3'>
              {species.map((specie, index) => (
                <div key={index} className='grid grid-cols-4 gap-4 bg-[#A7C4E4] p-2 mb-4 w-12/12 mx-auto'>
                  ...
                  <a className='text-center' href={`./species/update/${specie.id}`}>Modifier</a>
                  <button className='hover:bg-red-700' onClick={() => handleSpeciesDelete(specie.id)}>Supprimer</button>
                </div>
              ))}
            </div>
          </div>
